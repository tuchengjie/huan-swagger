...
* @Package com.huanshare.huanSwaggerCore.core
* @Description: plugin加载顺序，默认是最后加载
* @author tjc
...
public class MapApiReader implements ParameterBuilderPlugin {
     ...
     * 动态生成的Class名
     */
    private final static String BASE_PACKAGE = "com.xx.xxx.in.swagger.model.";
    ...
    private CtClass getFieldType(String type) throws NotFoundException {
        String mapType = "map",integerType = "integer",
        ...
        }else if(intType.equals(type) || integerType.equals(type)) {
            fileType = ClassPool.getDefault().get(Integer.class.getName());
        }else if(mapType.equals(type)) {
            fileType = ClassPool.getDefault().get(Map.class.getName());
        }
        return fileType;
    }
